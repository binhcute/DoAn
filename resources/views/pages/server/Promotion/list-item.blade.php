...
<tr>
    ...
    <td class="flex-column align-items-center justify-content-around">
        <a href="{{route('KhuyenMai.show',$item->promotion_id)}}" method="get">
            <i class="icofont icofont-paper" style="font-size:20px;color:green" title="Xem chi tiết"></i>
        </a>
        <a href="{{route('KhuyenMai.edit',$item->promotion_id)}}">
            <i class="icofont icofont-pencil-alt-5" style="font-size:20px;color:blue" title="Chỉnh sửa"></i>
        </a>
        <a data-url="{{URL::to('/XoaKhuyenMai',$item->promotion_id)}}" class="delete-item">
            <meta name="csrf-token" content="{{ csrf_token() }}">
            <input type="hidden" name="_method" value="delete">
            <i class="icofont icofont-trash" style="font-size:20px;color:red" title="Xóa"></i>
        </a>
    </td>
</tr>
@endforeach